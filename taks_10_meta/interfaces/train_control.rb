...
require './classes/car'
require './classes/passenger_car'
require './classes/cargo_car'
...
class TrainControl
  ...
  def set_route
    ...
    if @main_data.trains.at(ind).nil?
      ...
      if @main_data.routes.at(ind).nil?
        ...
        @main_data.trains.at(ind).route = @main_data.routes.at(ind)
        start
      end
    end
  end
  ...
  def add_car
    ...
    car = case train.type
          when 'cargo'
            CargoCar.new(manufacturer, capacity)
          when 'passenger'
            PassengerCar.new(manufacturer, capacity)
          else
            Car.new(manufacturer, capacity)
          end
  rescue ValidationError
    puts 'Попробуйте ещё раз!'
    retry
  ...
  def remove_car
    ...
    if train.nil?
      ...
      train.cars.each_with_index { |_car, index| puts "#{index + 1}: Вагон №#{index + 1}" }
      ind = gets.chomp.to_i
  ...
  def get_cars
    ...
    puts "Поезд #{train}:\n"
    puts 'Вагонов нет!' if train.cars.count.zero?

    train.iterate_cars do |car, i|
      puts "Вагон №#{i + 1}: Тип: #{car.type}, Производитель: #{car.manufacturer}, " \
           "Вместимость: #{car.capacity}, Свободно: #{car.free_space}, Занято: #{car.occupied_space}"
    end
  end
end
